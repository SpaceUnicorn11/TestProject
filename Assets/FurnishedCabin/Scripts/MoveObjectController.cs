		...
		{	
			...
			if (Physics.Raycast(rayOrigin,fpsCam.transform.forward, out hit,reachRange,rayLayerMask))
			{
				...
				if (moveableObject != null)		//hit object must have MoveableDraw script attached
				{
					...
					if (moveableObject.objectNumber != 10)
					{
						msg = getGuiMsg(isOpen);
					}
					else
                    {
						msg = "Press E/Fire1 to Win";
					}

					if ((Input.GetKeyUp(KeyCode.E) || Input.GetButtonDown("Fire1")) && moveableObject.objectNumber != 10 && player.GetComponentInChildren<PlayerLook>().enabled != false)
					{
						anim.enabled = true;
						anim.SetBool(animBoolNameNum,!isOpen);
						msg = getGuiMsg(!isOpen);
						game.GetComponent<GameController>().errorCounter ++;
					}
					else if ((Input.GetKeyUp(KeyCode.E) || Input.GetButtonDown("Fire1")) && moveableObject.objectNumber == 10 && player.GetComponentInChildren<PlayerLook>().enabled != false)
					{
						game.GetComponent<GameController>().Victory();
                    }
